...
function checkCssGuardrails(globalCss, homeCss) {
  ...
  requirePattern('Consultorias CTA must keep centering after generic button styles.', homeCss, /\.o-consultorias-destaque-cta\.o-btn\s*\{[\s\S]*?display:\s*flex[\s\S]*?margin-inline:\s*auto[\s\S]*?width:\s*min\(100%,\s*320px\)/);
